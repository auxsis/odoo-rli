...
class StockSettings(models.TransientModel):
    ...
    def get_partner_notify(self):
        user_list = []
        for partner in self.env['res.users'].search([('notify_user','=',True)]):
            if partner.notify_user:
                user_list.append(partner.name)
        return user_list
    ...
    def action_low_stock_send(self):
        ...
        su_id =self.env['res.users'].browse(self.env.uid)
