        ...
        display_pos_order_detail: function (order) {
            ...
            this.$('.reprint_order').click(function () {
                ...
                return rpc.query({
                    ...
                    method: 'search_read',
                    domain: [['id', '=', order.id]],
                    fields: ['receipt_xml', 'receipt_html']
                }).then(function (results) {
                    if (!results) {
                        return;
                    }
                    var results = results[0];
                    if (results['receipt_xml'] == false || results['receipt_html'] == false) {
                        return self.pos.gui.show_popup('dialog', {
                            title: 'WARNING',
                            body: 'This order have not save receipt, please select another order',
                        });
                    } else {
                        self.pos.report_xml = results['receipt_html'];
                        self.pos.posbox_report_xml = results['receipt_xml'];
                    ...
                    return self.pos.gui.show_popup('dialog', {
                        title: 'WARNING',
                        body: 'Your server turn off or your internet have problem, could not reprint receipt this order',
                    });
                });
            });
