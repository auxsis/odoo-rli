...
odoo.define('pos_retail.order', function (require) {
    ...
    models.Order = models.Order.extend({
        ...
        export_as_JSON: function () {
            ...
            if (this.receipt_xml) {
                json.receipt_xml = this.receipt_xml
            }
            if (this.receipt_html) {
                json.receipt_html = this.receipt_html
            }
            return json;
        },
    ...
    models.Orderline = models.Orderline.extend({
        ...
        set_unit_price: function (price) {
            var self = this;
            if (this.product && (parseFloat(price) < this.product.minimum_list_price) && !this.packaging) {
                return this.pos.gui.show_popup('number', {
                    'title': _t('Input price need to change'),
                    'value': 0,
