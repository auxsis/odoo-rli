...
class pos_order(models.Model):
    ...
    receipt_xml = fields.Char('Receipt xml', readonly=1)
    receipt_html = fields.Char('Receipt html', readonly=1)
    ...
    def create_from_ui(self, orders):
        ...
        pos_sale_extra = self.env['pos.sale.extra']
        for o in orders:
            data = o['data']
            lines = data.get('lines', {})
            sale_extra_by_product_id = {};
            for line in lines:
                line = line[2]
                if line.get('sales_extra', {}):
                    for id_string_extra, qty in line.get('sales_extra').items():
                        sale_extra = pos_sale_extra.browse(int(id_string_extra))
                        if not sale_extra_by_product_id.get(sale_extra.product_id.id):
                            sale_extra_by_product_id[sale_extra.product_id.id] = {
                                'qty': qty,
                                'price_unit': sale_extra.list_price,
                                'discount': 0,
                                'product_id': sale_extra.product_id.id,
                                'tax_ids': [6, 0, []]
                            }
                        else:
                            sale_extra_by_product_id[sale_extra.product_id.id]['qty'] += qty
            for product_id, value in sale_extra_by_product_id.items():
    ...
    def add_credit(self, amount):
        ...
        if self.partner_id:
            _logger.info('Begin add_credit %s to %s' % (amount, self.partner_id.name))
        ...
        for o in orders:
            ...
            if o['data']['name'] == order.pos_reference:
                ...
                if combo_items:
                    ...
                    _logger.info('call action_assign')
                    order_picking.action_assign()
                    _logger.info('call force_assign')
