...
class product_template(models.Model):
    ...
    sale_extra = fields.Boolean(compute='_get_sale_extra', string='Active Sale extra')
    ...
    sale_via_package = fields.Boolean('Sale via package')
    have_packaging = fields.Boolean(compute='_checking_have_packaging', string='Have packaging')

    @api.multi
    def _checking_have_packaging(self):
        for product in self:
            product.have_packaging = False
            if product.packaging_ids and len(product.packaging_ids) > 0:
                product.have_packaging = True
