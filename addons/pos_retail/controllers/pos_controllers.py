...
class dataset(DataSet):
    ...
    def call_kw(self, model, method, args, kwargs, path=None):
        retail = kwargs.get('retail', None)
        config_id = kwargs.get('config_id', None)
        if retail:
            del kwargs['retail']
            del kwargs['config_id']
        response = super(dataset, self).call_kw(model, method, args, kwargs, path)
        if retail:
            domain = kwargs.get('domain', [])
            fields = kwargs.get('fields', [])
            request.env['pos.parameter'].save_parameter(config_id, model, response, domain, fields)
        return response
...
class pos_controller(PosController):
    ...
    def pos_web(self, debug=False, **k):
        parameter_object = request.env['pos.parameter']
        config_parameter_object = request.env['ir.config_parameter'].sudo()
        ...
        first_install = config_parameter_object.get_param('pos_retail_first_install')
        if not first_install:
            request.env.cr.execute("DELETE FROM pos_cache_database")
            request.env.cr.commit()
            config_parameter_object.set_param('pos_retail_first_install', 'Done')
        ...
        if pos_session:
            ...
            if config.use_parameters:
                results = parameter_object.search([('config_id', '=', config.id)])
                if results:
                    for result in results:
                        if not session_info['parameters'].get(result.model_name, None):
                            session_info['parameters'][result.model_name] = [{
                                'domain': result.domain,
                                'datas': result.datas
                            }]
                        else:
                            session_info['parameters'][result.model_name].append({
                                'domain': result.domain,
                                'datas': result.datas
                            })
                else:
                    session_info['parameters'] = None
            session_info['use_parameters'] = config.use_parameters
            session_info['config_id'] = config.id
        context = {
